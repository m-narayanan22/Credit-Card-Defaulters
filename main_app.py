...
st.set_page_config(page_title = 'Credit Card Defaulters Classification',
                    page_icon = ':card:',
                    layout = 'centered',
...
def load_data():
    ...
    # Replace the garbage values from 'EDUCATION' and 'MARRIAGE' columns 
    cc_df['EDUCATION'].replace({0:4,5:4,6:4}, inplace=True)
    cc_df.EDUCATION.value_counts()

    cc_df['MARRIAGE'].replace({0:3}, inplace=True)
...
#     # cc_df['car_company'] = car_companies
...
selected_page.app(final_cc_df)
# This 'app()' function is defined in all the 'data.py', 'plots.py' and 'predict.py' files. 
